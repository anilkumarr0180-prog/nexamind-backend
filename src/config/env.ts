...
const envSchema = z
  ...
  .default(32000),
    ...
    // IMPORTANT:
    // Keep the Polar webhook secret exactly as provided by Polar.
    // Do NOT strip "whsec_" here.
    // @polar-sh/sdk@0.49.0 performs its own encoding internally.
  ...
  .superRefine((data, ctx) => {
    if (
      ...
      (!data.GROQ_API_KEY || data.GROQ_API_KEY.trim().length === 0)
    ) {
      ctx.addIssue({
